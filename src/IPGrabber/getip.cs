...
namespace IpGrabber
{
    class Main
    {
        ...
        {
            Console.WriteLine($"ipgr/pLan> Starting IPGR LAN host scan at {DateTime.Now}...\n");
            ...
            if(!verbose)
                Console.WriteLine("|"+new string('-',title_lng-1));
            Console.WriteLine($"\nipgr/pLan> Found a total of {host_count} open hosts of 256 on {ip}.\n");
            host_count = 0;
        }
            ...
            for(int i = 0; i < 4; i++)
            {
                try
                {
                    ...
                    if(reply != null)
                    {
                        ...
                        {
                            Console.WriteLine($"ipgr/pingHost> Reply from {reply.Address}: bytes={reply.Buffer.Length} time={reply.RoundtripTime}, TTL={reply.Options.Ttl}");
                        }
                        else if(reply.Status == IPStatus.TimedOut)
                        {
                            Console.WriteLine("ipgr/pingHost> Connection timed out...");
                        }
                        else
                        {
                            Console.WriteLine($"ipgr/pingHost> Connection failed: {reply.Status.ToString()}");
                        }
                    }
                    else
                    {
                        Console.WriteLine("ipgr/pingHost> Ping reply was null for some reason...");
                    }
                }
                catch(PingException ex)
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine($"[!]-[ipgr/errLogger] > An error occured : {ex.Message}");
                    Console.ForegroundColor = ConsoleColor.White;
                }
